...
| File | What it is |
|------|-----------|
| `index.html` | Main live page (hosted on GitHub Pages). Day-by-day fixture calendar (all 72 group games, Irish kick-off times) **plus** standings/leaderboard. Auto-scores and goal events are patched in by `scores.py` and pushed by a GitHub Actions workflow every 30 min. Nav abbreviated for mobile (Cal / Table / Groups / Results / Squads). |
| `squads.html` | Generated file — **do not edit directly**. 48-team squad browser with player cards showing club, caps, goals, captain badge, position. Generated by `squads.py`. |
| `previews.html` | Standalone page of 72 WhatsApp-style match previews, one per group game. Each has a "Copy to WhatsApp" button. Filterable by family member. Football-fact-led banter with light family references. **The PREVIEWS array is duplicated inside `index.html` — edits must be applied to both files.** |
| `standings.html` | Earlier standalone version of the standings/results page. Largely superseded by the section inside `index.html` — keep or delete. |
...
7. **Squad withdrawal tracking** — Wikipedia notes are shown in a modal per team, but withdrawn players are not automatically removed from the API-sourced roster. When a withdrawal occurs, patch `squads_cache.json` manually and regenerate (see Scripts above).
...
open index.html                  # main live page — calendar + standings + auto-scores
open previews.html               # WhatsApp match previews
open squads.html                 # squad browser (generated — run squads.py to update)
```
